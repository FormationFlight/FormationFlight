...
// Positions are deg * 1e7 (the v2 wire/MSP format).
static const int32_t ORIGIN_LAT_1E7 = 0;
static const int32_t ORIGIN_LON_1E7 = 0;

// ---- Four cardinal single-axis offsets at course_deg = 0 (due-north leader) ----

void test_ahead_at_course_zero_is_due_north() {
    FollowTarget t = slotToLatLon(ORIGIN_LAT_1E7, ORIGIN_LON_1E7, 0.0, /*long_m=*/10.0, /*lat_m=*/0.0);
...
void test_right_at_course_zero_is_due_east() {
    FollowTarget t = slotToLatLon(ORIGIN_LAT_1E7, ORIGIN_LON_1E7, 0.0, /*long_m=*/0.0, /*lat_m=*/10.0);
...
void test_behind_at_course_90_is_due_west() {
    ...
    FollowTarget t = slotToLatLon(ORIGIN_LAT_1E7, ORIGIN_LON_1E7, 90.0, /*long_m=*/-15.0, /*lat_m=*/0.0);
    TEST_ASSERT_INT32_WITHIN(50, 0, t.lat_1e7);
    TEST_ASSERT_TRUE(t.lon_1e7 < 0);
}
...
void test_behind_at_course_270_is_due_east() {
    ...
    FollowTarget t = slotToLatLon(ORIGIN_LAT_1E7, ORIGIN_LON_1E7, 270.0, /*long_m=*/-15.0, /*lat_m=*/0.0);
    TEST_ASSERT_INT32_WITHIN(50, 0, t.lat_1e7);
    TEST_ASSERT_TRUE(t.lon_1e7 > 0);
}
...
void test_ahead_bearing_wraps_near_360() {
    ...
    // something near 0/negative. Cross-check against geo::pointAtDistance()
    // called directly with distance=long_m, bearing=350.
    FollowTarget t = slotToLatLon(ORIGIN_LAT_1E7, ORIGIN_LON_1E7, 350.0, /*long_m=*/10.0, /*lat_m=*/0.0);

    double expectedLat = 0.0;
    double expectedLon = 0.0;
    geo::pointAtDistance(0.0, 0.0, 10.0, 350.0, expectedLat, expectedLon);
    TEST_ASSERT_INT32_WITHIN(5, (int32_t)std::lround(expectedLat * 1e7), t.lat_1e7);
    TEST_ASSERT_INT32_WITHIN(5, (int32_t)std::lround(expectedLon * 1e7), t.lon_1e7);
}

// ---- Round-trip against geo::pointAtDistance()'s own contract:
// independently derive the expected bearing/distance from
...
// that bearing/distance through pointAtDistance() lands on the exact same
// point slotToLatLon() produced. This checks slotToLatLon()'s derivation,
// not pointAtDistance() itself. ----

static void assertRoundTrips(double course_deg, double long_m, double lat_m) {
    FollowTarget actual = slotToLatLon(ORIGIN_LAT_1E7, ORIGIN_LON_1E7, course_deg, long_m, lat_m);
    ...
    double bearing_deg = course_deg + geo::toDeg(std::atan2(lat_m, long_m));
    bearing_deg = std::fmod(bearing_deg, 360.0);
    if (bearing_deg < 0.0) bearing_deg += 360.0;

    double expectedLat = 0.0;
    double expectedLon = 0.0;
    geo::pointAtDistance(0.0, 0.0, distance_m, bearing_deg, expectedLat, expectedLon);

    TEST_ASSERT_INT32_WITHIN(5, (int32_t)std::lround(expectedLat * 1e7), actual.lat_1e7);
    TEST_ASSERT_INT32_WITHIN(5, (int32_t)std::lround(expectedLon * 1e7), actual.lon_1e7);
}

void test_round_trip_ahead() { assertRoundTrips(0.0, 10.0, 0.0); }
